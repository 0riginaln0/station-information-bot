...
path = "C:\\Users\\first\\Desktop\\secretTocken.txt"
with open(path, "r", encoding='utf-8') as file:
    src = file.read()

bot = telebot.TeleBot(f'{src}', exception_handler=ExceptionHandler(), threaded=True, num_threads=4)
...
def start(message):
    ...
    bot.send_message(message.chat.id, 'Привет! Я - бот, который узнаёт время прибытия транспорта до остановки', reply_markup=keysmarkup)
...
def review_link(message):
    ...
    if re.match(stop_regexp, link_to_save) and not is_in_list(link_to_save, message.chat.id):
        ...
        bot.reply_to(message, 'Ссылка неправильная, она должна быть следующего формата: https://yandex.ru/maps/-/CCUrV4w63B')
    else:
...
def delete_stop_link(message):
    ...
    bot.send_message(message.chat.id, 'Выберите какую остановку удалить:', reply_markup=deleting_keyboard)
...
def get_stop_info(url, chat_id) -> str:
    ...
    if len(transportTypes) == 0:
        outputlist.append(f'{title} - не имеет информации о прибывающем транспорте\n Скорее всего вы предоставили неправильную ссылку.')
...
def findTransportTime(TransportType, TransportPostfix, outputlist, doc):
    ...
    list = doc.find('ul', class_="masstransit-brief-schedule-view__vehicles").find_all('li', class_=f'{TransportPostfix}')
    for item in list:
        outputlist.append(item.get_text(strip=True, separator=' ') + "\n")
    outputlist.append("\n")
    
def get_source_html(url, chat_id):
    driver_service = Service("C:\Games\Python\projects\transport_bot\chromedriver\chromedriver.exe")
